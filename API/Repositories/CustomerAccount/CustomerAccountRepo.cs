...
namespace API.Repositories.CustomerAccount
{
    public class CustomerAccountRepo : ICustomerAccountRepo, IDisposable
    {
        ...
        {
            var customers = await _context.CustomerAccounts.Where(c => c.UserId == userId).AsNoTracking().ToListAsync();
        ...
        public async Task<(bool, string, IEnumerable<CustomerBalanceDetailsDTO>?)>
        ...
        public async Task<IEnumerable<BuyAndSellTransactionDTO>> GetAllBuyAndSellTransactionsAsync(int userId)
        {
            try
            {
